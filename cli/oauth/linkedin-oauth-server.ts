...
export async function captureLinkedInCode(port: number = 8080): Promise<string> {
  return new Promise((resolve, reject) => {
        ...
        res.end(`
          <html>
            <body style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100vh; margin: 0; background: #18181b; color: #fafafa;">
              <h1 style="color: #ef4444;">✗ LinkedIn Authorization Denied</h1>
              <p style="color: #a1a1aa;">Error: ${error}</p>
              ${errorDescription ? `<p style="color: #71717a;">${errorDescription}</p>` : ""}
              <p style="color: #71717a; font-size: 0.875rem;">You can close this window.</p>
            </body>
          </html>
        `);
        cleanup();
        reject(new Error(`LinkedIn authorization denied: ${error} - ${errorDescription}`));
        return;
      }
    ...
    const timeout = setTimeout(() => {
      ...
      reject(
        new Error(
          "LinkedIn OAuth callback timed out after 2 minutes. Please try again.",
        ),
      );
    }, CALLBACK_TIMEOUT_MS);
    ...
    server.listen(port, () => {
      log.info(
        `  Waiting for LinkedIn callback on http://localhost:${port} (times out in 2 min)`,
      );
    });
